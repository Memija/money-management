...
describe('rowsToTransactions', () => {
  ...
  describe('Sparkasse', () => {
    it('parses Sparkasse CSV with Buchungstag/Valutadatum/Beguenstigter headers', () => {
      ...
      expect(txs[1].amount).toBe(2800.5)
      expect(txs[1].type).toBe('income')
    })
  })
    ...
    it('parses Volksbank CSV with standalone "Umsatz" column', () => {
      ...
      const rows = [
        [
          ...
          'Primanota',
          'Zahlungsempfänger',
          'ZahlungsempfängerKto',
          'ZahlungsempfängerIBAN',
          'ZahlungsempfängerBLZ',
          'ZahlungsempfängerBIC',
...
describe('parsePdfText', () => {
  it('parses dual-date German bank statement format', () => {
    const text =
      '20.05.2026 20.05.2026 Dauerauftrag Trade Republic -500,00 EUR 15.05.2026 15.05.2026 Überweisung Gehalt Mai 2026 3.200,00 EUR'
