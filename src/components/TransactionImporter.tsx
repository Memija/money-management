...
const TransactionImporter: React.FC = () => {
  ...
  const handleFileChange = useCallback(
    async (file: File) => {
      ...
      try {
        if (method === 'excel' || file.name.match(/\.(xlsx|xls)$/i)) {
          ...
          setTransactions(txs);
        } else if (method === 'csv' || file.name.match(/\.csv$/i)) {
  ...
  const handlePaste = () => {
    ...
    try {
      ...
      const txs = rowsToTransactions(rows, institutionName);
      if (txs.length === 0) throw new Error('No transactions could be parsed from the pasted data.');
      setTransactions(txs);
    } catch (err: any) {
