              ...
              {categoryMap.slice(0, 6).map((cat) => (
                <div key={cat.name} className={styles['category-item']}>
                  <div className={styles['category-name-container']}>
                    <div
                      className={`${styles['category-dot']} ${cat.colorClass}`}
                    />
                    <span className={styles['category-name']}>{getCategoryLabel(cat.name)}</span>
                  </div>
              ...
              {filteredTx.slice(0, 100).map((tx) => (
                <div key={tx.id} className={styles['transaction-item']}>
                  ...
                  <div className={styles['tx-right']}>
                    <p className={tx.type === 'income' ? styles['amount-positive'] : styles['amount-negative']}>
                      {tx.type === 'income' ? '+' : ''}
